...
module LifeImputedIncomeCostCalculatorTests =
    ...
    let ``Imputed Income Monthly Cost is 0 when Imputed Income Coverage is 0``() =
        ...
        let lifeImputedIncomeCoverageCalculator = { new ILifeImputedIncomeCoverageCalculator 
                                                    with member this.GetImputedIncomeCoverage(x) = decimal 0 }

        let lifeImputedIncomeCalculator = new LifeImputedIncomeCalculator(lifeImputedIncomeCoverageCalculator)
        let actual = lifeImputedIncomeCalculator.GetImputedIncomeMonthly(selectedPlanAndTier, DateTime.Parse "1/1/2016")
    ...
    let ``Imputed Income Monthly Cost greater than 0 when Imputed Income Coverage is greater than 0``() =
        let selectedPlanAndTier = new SelectedPlanAndTier(ImputedIncomeCosts = imputedIncomeCosts, User = new User(BirthDate = DateTime.Parse "1/1/1980"))
        let lifeImputedIncomeCoverageCalculator = { new ILifeImputedIncomeCoverageCalculator 
                                                    with member this.GetImputedIncomeCoverage(x) = decimal 30000 }
